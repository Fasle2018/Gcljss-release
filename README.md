...
- **`CCX` 完成时命令行按 `GG` 同款格式输出统计**：
  ```
  立管统计:(3.8-1.3)*4+(3.9-1.3)。
  长度统计:4.86+6.27+6.1+5.79+2.69+1.84+1.99+5.71+5.72+2.98+0.4+1.38=45.73。
  ***共统计17个图元***
  ```
  立管按**原始表达式**统计（相同表达式合并为 `*N`、表达式加括号，避免被 `+` 拆项），长度沿用写入单元格的数值合计；空类目行不输出（没有块 / 文字时就没有「数量统计」行），也不再另起一行回显写入的单元格字符串。
- **`CCX` 按键语义统一为三状态状态机（命令期间始终是同一套习惯）**：任一时刻只有一个状态激活，提示里**只列另外两个分支**，按键直接切换、不必先退出当前状态 ——
  - 起点 / 画线：`T` = 挑拣对象、`G` = 布置立管；
  - 挑拣对象：`B` = 返回画线、`G` = 布置立管；
  - 立管布置：`T` = 挑拣对象、`B` = 返回画线（提示文本保持「请输入立管位置,或(挑拣对象(T)/返回画线(B)):」）；
  - **全状态**：回车 / 空格 / 右键 = 完成并写入活动单元格；`ESC` = 取消本次命令（只删除本次补画 / 布置的对象，Excel 侧零接触）。
  - 分支键都支持「单键免回车」与「键入 + 回车」两条路；切换状态时输入浮窗随之打开 / 关闭（切到挑拣或画线即关闭，切回布置再打开）。
  - 注：焦点停在立管输入框里时，回车 = 确认该输入并登记历史；要「完成 / 取消」请先在绘图区或命令行按一下，让 CAD 取回焦点。
- **`CCX` / `BZLG` 的输入浮窗：新输入的值立即进入下拉历史**（原实现要等命令结束才追加，须下次打开窗体才看得见）；同时把登记时机收紧为「回车确认 / 立管落点提交后」—— 鼠标划出窗体去图中取点，不会再把手打到一半的 `4.2-` 写进历史。
- **`CCX` 顺带修复与加固**：空表达式 + 左键落点不再被误当成「完成」提前写表；布置过程出错不再改判「完成」；单键与落点同帧时注入的 `ESC` 余波曾可能被误判成用户取消（会删掉本次全部新建对象）→ 改为一次性核销、只重新提示；四处提示补「未识别关键字」护栏；单键键盘钩子的前台判据细化为「前台顶层窗口须是 CAD 主窗口或文档窗口」，在浮窗里打字不再被吞键；命令启动日志新增 `已加载程序集`（路径 / 时间戳 / 版本），便于确认实机加载的是哪一份产物。
- 版本号全项目统一 3.0.71.0。

### 3.0.70.0（近期）

- **`GL` 过滤选择：选择参照图元后，筛选条件不再被锁定（本版核心）**。此前流程是「选参照 → 条件被锁死 → 框选范围」，中途想改条件只能退出重来。本版把「圈范围」与「按条件筛选」解耦：从选参照到确定范围的**全程都可随时勾选 / 取消**筛选条件（对象类型 / 图层 / 颜色 / 线型 / 高级），**回车确定范围的那一刻**按最新条件出结果。框选时范围内的对象都会高亮、回车后只保留匹配项（与 `GLS` 的手感一致）。
  - 顺带修好一个隐性缺陷：此前「全部」总开关**未随其它条件一起锁定**，点它会改变条件并与框选阶段的过滤冲突，导致结果与条件对不上。
- **`GL` / `GLS` / `TQ` 全图搜索不再跨布局**：命令行直接回车走「全图」时，此前会把其它布局（图纸空间）里同条件的对象一并选中并缩放过去；现只保留当前布局 / 空间的对象。
- **`TQ` / `EM` 的文本筛选与 `GL` / `GLS` 对齐（修漏选）**：多行文字此前存在「内容相同、但格式（加粗 / 字体等）或大小写不同就筛不出来」的问题 —— 筛选时先按含格式码的原文做了一次系统级预筛，与最终比较口径不一致。现文本判定统一交给同一套比较器，此类对象可正常筛出。
- **闭合曲线范围支持圆弧段边界**：点选**含圆弧段的多段线**作为搜索范围时，圆弧被当成直边（弦）处理，边界附近的对象会误选 / 漏选；现按弧线离散采样，边界与实际形状一致。
- **`GL` / `GLS` 大范围筛选可中止**：新增进度显示，候选对象较多时可按 `ESC` 中断（此前大图全图搜索可能长时间无响应且无法中止）；`ESC` 中断后会清理残留框选高亮。
- **`GLS` 的选择步骤异常时不再遗留选项窗口**（源图元 / 范围选择已纳入统一的异常退出路径）。
- **单键免回车（`M` / `T` / `B`）的键盘钩子交接加固**：极少数情况下钩子卸载失败仍会继续安装，可能导致整个 CAD 会话吞键；现改为卸载失败时拒绝安装并提示重启 CAD。
- **清理编译器与 IDE 告警**：消除 XML 注释错位告警（`BC42300`）、2 处冗余赋值提示（`IDE0059`），以及 Ribbon 图标「已有一个指向该文件的链接」告警（浅色与深色两套图标存在 36 个同名文件、链接名冲突所致）；图标显示不受影响。
- 版本号全项目统一 3.0.70.0。
